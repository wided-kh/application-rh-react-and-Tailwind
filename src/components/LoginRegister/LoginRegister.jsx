...
const LoginRegister = ({ setIsAuthenticated }) => {
    ...
    const [action, setAction] = useState(''); // État pour gérer l'affichage des formulaires
    ...
    const registerLink = () => {
        setAction(' active'); // Affiche le formulaire d'inscription
    };

    const loginLink = () => {
        setAction(''); // Affiche le formulaire de connexion
    };
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            localStorage.setItem('token', response.data.token); // Enregistrez le token
            navigate('/main'); // Redirigez vers la page principale après connexion
        } catch (error) {
    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            setIsAuthenticated(true); // Mettez à jour l'état d'authentification
            localStorage.setItem('token', response.data.token); // Enregistrez le token
            navigate('/main'); // Redirigez vers la page principale après inscription
        } catch (error) {
                ...
                <div className={`form-box register ${action === ' active' ? 'block' : 'hidden'}`}>
                    <form onSubmit={handleRegister}>
                        ...
                        <div className="login-link text-white text-sm text-center mt-4">
                            <p>Vous avez déjà un compte? <a href="#" onClick={loginLink} className="text-white hover:underline">Se connecter</a></p>
                        </div>
                        {message && <p className="text-red-500 text-center mt-2">{message}</p>}
                    </form>
                </div>
